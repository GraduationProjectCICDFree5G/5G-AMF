pipeline {
    ...
    stages {
        ...
        stage('Pulling base image from Dockerhub') {
            steps {
                    sh 'docker pull 5ggraduationproject/amf-base'
            }
        }
        ...
        stage('docker build') {
            steps {
                sh(script: """
                    docker images -a
                    docker build -t 5ggraduationproject/5g-amf:latest . 
                    docker images -a
                """)
            }
        }
        ...
        stage('Scan Image for Common Vulnerabilities and Exposures') {
            steps {
                sh 'trivy image 5ggraduationproject/5g-amf --output trivy-report.json'
            }
        }
        stage('Pushing to Dockerhub') {
            steps {
                sh 'docker push 5ggraduationproject/5g-amf:latest'
            }
        }
    ...
    post {
        always {
            ...
            sh 'docker rmi 5ggraduationproject/5g-amf'
            sh 'docker rmi 5ggraduationproject/amf-base'
        }
    }
}